...
Dans le module _react-hooks-avances_ nous avons vu le pattern `props drilling`.
Le fait de passer toutes les données de `props` en `props` dans les enfants.
Plus l'application grossie, plus il y a de données passées en `props` à gérer.
...
Nous voulons maintenant que le changement de thème soit géré dans
`ThemeProvider` et donc accessible depuis les enfants (par exemple dans le
composant `CheckBox`)
...
`ThemeProvider`. Au lieu de retourner uniquement `theme` il retournera un state
qui contient : `theme` et `setTheme`.
